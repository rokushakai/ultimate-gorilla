...
- **[v0.57.1] 仲間装備商人・仕様整合／購入安全性監査**（§138）
  ...
  - **COMPANION_GEAR_SHOP_ITEMS更新**: スターター→専用4種（各60G）
  - **報酬gear DOM完全除外**: renderCompanionGearShopはCOMPANION_GEAR_SHOP_ITEMSのみ描画
  - **`getCompanionGearPurchaseStatus(gearId)`**: 詳細状態純粋関数（joined/owned/affordable/reason等）
  - **`_companionGearPurchaseLock`**: 連打防止変数（全出口でfalseリセット）
  - **`buyCompanionGear()` 強化**: lock・status最終再確認・saveGame1回
  - **`renderCompanionGearShop()` 更新**: ホワイトリスト・未加入🔒表示
  - **デバッグ16本（§138）**: `btn-debug-v571-*`
  - **BGM・捕獲条件・既存8gear効果・saveGame/loadGame: 変更なし**
- **[v0.57] 仲間装備商人販売・安全なショップ基盤**（§137）
  - **`renderMerchantMain()` 拡張**: 「🎽 仲間装備を買う」ボタン追加
  - **デバッグ4本（§137）**: `btn-debug-v57-*`（§138で参照更新済み）
...
- **[v0.51] 拡張フィールド・ワープ広場・ステージ別敵強化**（§129）
  ...
  - ステージ別倍率スケーリング (×3.0〜×13.0)・敵名 Lv.N 表示
  - ステージ別テーマCSS・帰還座標の動的管理 (normalReturnX/Y)
...
- **[v0.46] フィールド仲間追従表示・仲間アイコン人型化**（§121）
  ...
  - **全UI統一**: 戦闘コマンド/固有/まほうメニュー・酒場4箇所・ステータス2箇所・冒険の記録 → `icon + 名前` に変更
  - **ハルミ✨問題解消**: フィールド上では人型アイコン👧を使用し地形タイル「X（✨）」と区別
  - **デバッグ2本（§121）**: 仲間アイコン一覧確認 / 1人追従確認（シュリタニのみ）
...
- **[v0.44] 仲間サイドストーリー第一段階**（§113）
  ...
  - **`companionSideStoryFlags`**: 永続完了フラグ（saveGame/loadGame/newGame対応）
  - **酒場「📖 仲間の物語」**: renderTavernMain()→renderTavernStories()→startCompanionSideStory()フロー
  - **物語モーダル**: `#companion-story-modal`（title/speaker/text/progress）、ライン毎に読み進め
  - **完了処理**: `completeCompanionSideStory(cid)` 冪等・再読可能・初回のみトースト
  - **ステータス/冒険の記録**: 各仲間ごとに物語状態を表示
...
- **[v0.41] 仲間装備2種類目・装備選択アップデート**（§107）
  ...
  - **`ensureCompanionGearState()` v1→v2移行**: 既存セーブに特化装備4種を各1個追加配布
  - **ステータス画面**: 全装備バッジ表示（★装備中/装備可能/未所持）・装備袋仲間別グループ
  - **おすすめ一括装備（汎用型）**: スターター4種を明示的に選択
  - **デバッグ4本 (§107)**: v1→v2移行確認 / 新装備全員装備 / ジュリタニ比較 / ハルミ比較
...
> v0.57.1（§138 仕様整合監査）実装済み。スターター4種分類A確認→ショップ廃止、ショップ専用4種追加（各60G）。getCompanionGearPurchaseStatus追加。lock・joined確認・報酬DOM除外・debug16本。BGM/捕獲条件変更なし。
> v0.57（§137 仲間装備商人販売）実装済み。renderMerchantMainに「仲間装備を買う」追加。
