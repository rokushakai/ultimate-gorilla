...
1. **図鑑のUMA詳細ステータス表示** — 捕獲済みUMAをタップでHP/攻撃力/捕獲率を確認。工数小。
2. **ストーリーイベント・NPC会話** — フィールドに固定NPCを追加してヒントや台詞を表示。工数中。
3. **BGM/SE** — Base64でインライン埋め込み可能。工数中。
