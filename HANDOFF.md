...
1. **NPC固有イベント深化** — 仲間加入フラグ別の追加セリフや連続クエスト的な展開。工数中。
2. **BGMメロディ改善** — 現行メロディの音楽的改善や音量調整。工数小。
3. **仲間システム拡張** — フィールド追従・固有イベント・3分岐加入。工数大。
4. **ステージ5（ゴリラ山頂など）** — v0.13以降で検討。GAME_DESIGN.md §55 将来候補参照。

## v0.12 実装済み横スクロール全体仕様
...
## v0.11.3 横スクロールからの帰還仕様

| 方法 | 場所 | 動作 |
|---|---|---|
| 🏠 帰還ゲート | 各ステージ x=0 | 確認モーダル → switchToNormalMap() |
| ゴール画面 | 🏁 ゴールタイル | ゴールモーダルの「🏠 通常マップへ戻る」→ switchToNormalMap() |
