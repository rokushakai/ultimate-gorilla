...
1. **図鑑コンプリート報酬** — 全UMA捕獲時の称号/演出。工数小〜中。
2. **仲間別クリア後セリフ** — ジュリタニ/シュリタニ/ノリオ/ハルミのクリア後固有セリフ。工数小。
3. **NPC固有イベント深化** — 仲間加入フラグ別の追加セリフや連続クエスト的な展開。工数中。
4. **BGMメロディ改善** — 現行メロディの音楽的改善や音量調整。工数小。
