...
- **`formatKingDialogueText()` を `openNpcModal()` 内でのみ呼ぶ設計にした理由**: 王様NPC（tileChar="S"）に限定した変換であり、他のNPCや他のUI要素（エンディング・alert・toast）には一切影響しない。変換関数を呼び出し箇所を1か所に集中させることで、将来NPC会話文の変更があっても `openNpcModal` の1行だけ変更すればよい。

- **XSSエスケープを `formatKingDialogueText` 内で行う理由**: 主人公名はユーザー入力（`normalizePlayerName` でコントロール文字は除去するが `<>&"` は残る）であり、置換後の文字列が `openNpcModal` の `innerHTML` に直接注入される。置換関数内でエスケープすることで、呼び出し側がXSSを意識せずに使える設計にした。

- **partyTrail 上限を `> 4` にした理由**: partyTrail の各エントリが各仲間の「1歩前の位置」に対応する（trail[0]=仲間0の1歩前、trail[1]=仲間1の1歩前…）。4人パーティ対応のためには trail を最大4エントリ保持する必要がある。既存の `renderField()` の trail 参照ループは `companions.length` を上限にしているため、trail が増えても4人分まで自動対応する。

- **`joinAllCompanions()` で `COMPANION_DATA` の順序を尊重する理由**: パーティに加わる順序が仲間の追従位置（trail インデックス）に影響する。`COMPANION_DATA` の定義順（ジュリタニ → シュリタニ → ノリオ → ハルミ）でパーティに追加することで、全員合流時の並び順を常に一定に保つ。

- **設定モーダルのバックドロップ close を `bindEvents()` で1回だけ登録する理由**: `renderSettingsBody()` は設定を変えるたびに再描画される（BGMトグル等）。再描画のたびにリスナーを追加すると多重登録になり、1回のクリックで複数回 `closeModal` が呼ばれる。`bindEvents()` は初期化時に1回だけ呼ばれるため、ここに置くことで多重登録を防ぐ。
...
- **有効移動カウントを関数末尾に移した理由**: v0.48 ではカウント処理を `movePlayer()` の先頭付近（NPC接触判定の直後）に配置していた。しかしその時点では `tile` がまだ取得されていないため、H/M/G/T/D/R/K/E/S/N/V/B/U/A/C/J/X などのイベントタイルを踏んで早期 `return` しても、その前にカウントが加算されてしまっていた。関数末尾（エンカウント判定の直後）に移動することで、すべての早期 return の後でのみカウントされるよう保証する。「ここに到達できる」コードパスは草地・道の非イベントマスのみ。

- **`syncAdventureGuideObjective()` を純粋な「冪等リセッター」として設計した理由**: NPC接触前に呼ぶことで「接触時に目標が既に変わっていれば会話を開かずNPCを消す」という動作を保証する。この関数は副作用を持つが（6変数を変更する）、同じ状態で複数回呼んでも結果が変わらない冪等性を持つ。呼び出し側は「objective が変わっていない場合は何もしない」前提で呼べばよく、判定コストは `getCurrentAdventureGuide()` 1回呼び出しのみ。

- **`trySpawnAdventureGuideNpc()` の内部 `renderField()` を削除した理由**: `movePlayer()` はスポーン後にも `renderField()` を呼ぶため、スポーン関数内でも呼ぶと1回の移動で2回 renderField が実行されていた。スポーン関数は「変数を書き換えるだけ」にし、描画責任は呼び出し元に委ねる設計に統一した。

- **`openAdventureGuideNpcModal()` の talkLock 設計**: JavaScript はシングルスレッドのため、1回のコールスタック内での二重呼び出しは理論上不可能。しかし `movePlayer()` の NPC 接触判定が「NPC位置に移動した際に毎回呼ばれる」ため、タイミング次第でモーダルが閉じる前に再接触判定が走ることがある（モーダルを閉じながら同時にそのマスに留まるような状況）。talkLock はそのような境界ケースをカバーするベルト・サスペンダー防御。

- **`resetAdventureGuideNpcState()` を3箇所（switchToSide/switchToNormal/loadGame）に追加した理由**: セーブデータをロードしたとき、またはマップが切り替わったとき、NPCの位置情報がメモリ上の不正な状態のまま残ると、ロード後の renderField でフィールドに幽霊のような🧭が表示される可能性がある。モジュール変数は永続化されないが IIFE スコープ内で生き続けるため、ライフサイクルの境界でリセットを保証する必要がある。

---

## v0.48 (2026-07-20) — 冒険ナビゲーションシステム (§124)

### 設計判断・確認事項

- **`getCurrentAdventureGuide()` を独立した純粋関数として設けた理由**: 既存の `getHintPriority()` + `getProgressHint()` は有料ヒント屋の文脈で設計されており、テキストが「謎めいたヒント」トーン。NPC会話・ペーパービューガイドは「直接的な目標提示」が必要なため、同じ条件分岐ロジックを持ちながら別関数として並行実装した。`getHintPriority()` および `getProgressHint()` は一切変更しない。

- **旅の案内人NPCを「仮想エンティティ」として実装した理由**: 通常マップのタイル文字（RAW_MAP）を変えると、セーブデータとの整合性・`loadGame()` との互換性を考慮する必要が生じる。NPCをモジュール変数（`_adventureGuideNpcX/Y`）で管理し、`renderField()` 内で条件付きオーバーライドする設計にすることで、saveGame/loadGame への変更を完全にゼロにした。

- **出現閾値を15歩にした理由**: 10歩では頻繁すぎてプレイヤーの邪魔になる可能性があり、20歩では次の目標を見失ったプレイヤーが迷う時間が長くなる。15歩はフィールドを1往復半する距離に相当し、「迷っているなら15歩で気づけるはず」という経験則から設定。

- **NPC配置の優先順位（草地・道のみ、仲間トレイルより高優先）**: 既存の描画優先順位（プレイヤー > アイテム > 宝箱 > TERRAIN_EMOJI > 仲間トレイル）の最後から2番目に挿入。NPCが仲間アイコンと重なる場合はNPCを優先する設計にした。これはNPCが「伝えるべき情報を持っている」という意味でゲーム内の重要度が高いため。

- **目標変化時の即時リセット設計**: objective が変わった瞬間にステップカウントとNPC座標をリセットすることで、「古い目標のNPCが新しい目標達成後もフィールドに残る」問題を防ぐ。これにより、例えばステージ1をクリアした直後に古い案内人が消えて新しい案内人（次のステージ案内）が新鮮に出現する。
...
- **`icon` vs `emoji` の分離**: `emoji` フィールド（💪/🪤/📈/✨）は能力アイコンとして既存コードが広く参照している（UMAデータ、魔法データなども同じ `.emoji` パターン）。仲間本人識別用には新規 `icon` フィールドを追加し、既存 `emoji` の意味を保つことで、既存機能を壊さずに人型化を達成した。

- **描画優先度の修正理由**: §78 v0.26 時点では `trailMap` チェックがアイテム・宝箱チェックより前に実行されていた。そのため仲間が宝箱（🎁 など）の上に立った場合に宝箱が隠れる問題があった。§121 で `key` 変数を先に宣言し、チェック順を「プレイヤー → アイテム → 宝箱タイル → TERRAIN_EMOJI（NPC/施設含む）→ 仲間（草地/道のみ）→ デフォルト地形」に変更した。

- **仲間を草地・道のみに表示する理由**: プレイヤーは施設（H/M/G/T）・NPC（D/R/K/E/S/N）・ゲート（V）を踏んで移動する。これらのタイルに仲間が表示されると NPC や施設の存在が分かりにくくなる。草地（`.`）と道（`,`）のみに表示することで視認性を保つ。プレイヤーが施設タイルを踏んで離れると、次の草地・道タイルから追従表示が再開されるため自然に見える。

- **ハルミ✨の衝突問題**: ハルミの能力アイコン `✨` は地形タイル `"X"`（チグナスのかぶと伝説宝箱）の絵文字と同一だった。§78 v0.26 の実装では `cData.emoji` を使っていたため、フィールドで両者の区別が困難だった。`icon: "👧"` を使うことで解消した。
...
- **actionKey設計**: `getCompanionEquipmentBonus(cid, type, actionKey)` の3引数目は後方互換。既存の2引数呼び出し（actionKeyなし/undefined）は `damageBonus`/`healBonus` だけを返し、アクション別ボーナスはゼロ扱い。これにより既存戦闘コードを全て洗い直さなくても新ボーナスを段階的に追加できる。
- **会心倍率とactionKeyの関係**: `special1DamageBonus`（会心の腕輪+5）は会心倍率×1.6の計算後にフラット加算。v0.40設計方針（装備が倍率に乗らない）を継承。ジュリタニの `runCompanionSpecialAction()` 内で `actionKey="special1"` を渡すと、会心時のみ倍率が乗り、さらに+5がフラット加算される。
- **v1→v2移行の冪等性**: `ensureCompanionGearState()` の v1→v2ブロックは `version >= 1 && version < 2` ガードで保護。3回呼んでも1回分のみ配布。v0→v1→v2と連鎖する場合も各ブロックが独立に動作するため問題なし。version=0からの初回ロードは v0→v1→v2 の両ブロックを順次通過し、8種全て配布してversion=2に到達する。
- **おすすめ一括装備の明示化**: v0.40時代のコードは `Object.keys(COMPANION_GEAR_DATA)` を舐めて仲間別に最初に見つかったアイテムを装備させていた。v0.41で特化装備を追加するとJSオブジェクトのキー順次第で会心の腕輪が選ばれる可能性があったため、`preferred map` で汎用型4種（スターター）を明示的に固定した。
- **dup-checkデバッグハンドラーの更新**: v0.40.1で version=0→1をチェックしていたが、v0.41追加後は version=0→2が正常動作になるため、dup-checkも全8種×各1個＋version=2を検証するよう更新した。
- **ステータス画面装備UI刷新**: 旧実装は仲間ごとに「現在の装備」1行のみを表示。v0.41で各仲間の全装備をループして「★装備中/装備可能/未所持」バッジと効果説明行を表示する形式に刷新。`data-gear-action`/`data-gear-cid`/`data-gear-id` 属性を用いた既存イベントバインディングパターンを維持。
