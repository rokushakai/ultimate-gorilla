...
### Version 0.26 候補 — フィールド仲間追従表示

パーティ中の仲間を通常マップで主人公の後ろにドラクエ風に並べて歩かせる。

- 主人公の移動履歴（直近2ステップ）を記録し、1歩前の位置に仲間1、2歩前に仲間2を描画
- 通常マップのみ対象（横スクロールは別途検討）
- 戦闘システムには触らない
- renderField() / RAW_MAP / カメラ処理に影響するため、専用ブランチで着手すること
- 仲間能力・加入処理・パーティ上限は変更しない

### Version 0.27 候補 — 仲間の戦闘自動参加

仲間が戦闘中に自動で攻撃・補助する（仲間ごとのコマンド選択は入れない段階）。

- 既存の主人公コマンド戦闘（たたかう / まほう / つかまえる / うたう等）を壊さない形で段階導入
- 仲間ごとの特性（ジュリタニ→会心攻撃, シュリタニ→捕獲補助, ノリオ→EXP加算, ハルミ→魔法補助）を戦闘中にも反映
- finishBattle() / doFight() / doRun() には最小限の変更のみ

### Version 0.28 候補 — 仲間ごとの戦闘コマンド選択

主人公 → 仲間1 → 仲間2 → 敵 のような行動順にし、仲間ごとにコマンド選択できるようにする。

- 戦闘UIの大改修になるため、v0.27 が安定してから着手
- 仲間のHP/MP管理・死亡処理・状態異常も追加が必要
- GAME_DESIGN.md に詳細仕様を書いてから実装開始すること
