...
- **v0.56本体18項目監査**: すべてA（通常プレイ本体実装済み）✅
- **通常プレイ経路確認**: 第3話→演出→close→toast→酒場→開始の導線成立 ✅
- **既存最終サイドストーリー確認**: チンパンジーの聖域・`openStageWarpModal(6)` ✅
- **debug17本監査**: すべてproduction関数使用確認 ✅
- **デバッグ12本** (§136): `btn-debug-v561-*` — 通常経路各項目のproduction検証 ✅
...
- **`COMPANION_DATA.icon` 追加**: 仲間本人アイコン（人型）を一元管理（ジュリタニ🧑 / シュリタニ👩 / ノリオ👨 / ハルミ👧） ✅
- **`renderField()` 描画優先順位修正**: 仲間追従をNPC/施設/ゲートより低優先（草地・道のみに表示） ✅
- **フィールド追従**: `cData.icon` を使用（人型アイコン） ✅
- **戦闘コマンドタイトル**: `icon + 名前` に変更 ✅
- **固有コマンドサブメニュー**: `icon + 名前` に変更 ✅
- **まほうサブメニュー**: `icon + 名前` に変更 ✅
- **酒場（募集/パーティ確認/外す/加入演出）**: `icon + 名前` に変更 ✅
- **ステータス（仲間セクション/仲間物語カード）**: `icon + 名前` に変更 ✅
- **冒険の記録（仲間セクション）**: `icon + 名前` に変更 ✅
