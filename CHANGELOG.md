...
### Fixed
- **RAW_MAP row19 行幅27文字→26文字修正**: `"#####.,,,,,,,,,,,,,,,,.###"` に修正（padEnd/slice で機能的には問題なかったが一貫性のため）
- **`shouldSkipStageEnemyScaling()` 追加 (§130)**: 究極チンパンジー（`ultimate_chimpanzee`）・究極ゴリラ（`final: true` フラグ）をステージスケーリング除外。stage6 ×13倍が掛かっていた問題を修正
- **`_stageWarpTransitionLock` 追加 (§130)**: ワープ移動ボタン連打による二重入場を防止するフラグ。`btn-stage-warp-enter` 押下時にロック、`switchToNormalMap()` 呼び出し時に解除
- **ステージ1案内テキスト更新 (§130)**: `visit_side_gate`/`stage1_explore` の shortText・locationText にワープ広場への言及を追加

### Added
- **デバッグ14ボタン (§130)**: マップサイズ比較・全行幅確認・6ワープ出現数確認・ワープ入場先一致確認・解放境界確認・連打防止確認・normalReturn座標安全確認・全ステージ帰還位置確認・スケーリング累積防止確認・特殊敵スケーリング除外確認・捕獲率level非参照確認・敵Lv表示境界確認・6テーマ視覚差確認・theme class残留防止確認
...
### Added
- **`COMPANION_DATA.icon` フィールド**: 仲間本人識別用の人型アイコン（ジュリタニ🧑 / シュリタニ👩 / ノリオ👨 / ハルミ👧）
- **デバッグ2本（§121）**: 仲間アイコン一覧確認（人型チェック） / 仲間1人追従確認（シュリタニのみ）

### Changed
- **`renderField()` 描画優先順位修正**: 仲間追従をアイテム・宝箱・NPC/施設/ゲートより低優先に変更。草地（`.`）・道（`,`）のみに仲間を表示
- **フィールド追従**: `cData.emoji` → `cData.icon`（人型アイコン使用）
- **戦闘コマンドタイトル**: `cData.emoji + name` → `cData.icon + name`
- **固有コマンドサブメニュー**: `cData.emoji + name` → `cData.icon + name`
- **まほうサブメニュー**: `cData.emoji + name` → `cData.icon + name`
- **酒場（仲間募集/パーティ確認/外す/加入演出）**: `c.emoji + name` → `c.icon + name`（4箇所）
- **ステータス（仲間セクション/仲間物語カード）**: `.emoji + name` → `.icon + name`（2箇所）
- **冒険の記録（仲間セクション）**: `cd.emoji + name` → `cd.icon + name`
- **デバッグセクションヘッダー**: `§78 v0.26` → `§121 v0.46` に更新

### Fixed
- 仲間が宝箱・NPCタイルより前面に描画されていた問題を修正
- ハルミ `✨` が地形タイル「X」（✨）と視覚的に混同されていた問題を解消（フィールド上では人型アイコン使用）
...
### Added
- **特化装備4種を探索報酬として実装**: ステージ2〜5の初回クリア時に専用特化装備を付与（自動配布→報酬方式へ変更）
  - ステージ2: ⚡ 会心の腕輪（ジュリタニ） / ステージ3: 🕸️ 網師のベルト（シュリタニ）
  - ステージ4: 📓 研究ノート（ノリオ） / ステージ5: 🙏 祈りのブローチ（ハルミ）
- **`COMPANION_GEAR_REWARD_DATA`** 定数: 特化装備4種の入手元情報
- **`state.companionGearRewardFlags`**: 特化装備取得済みフラグ（二重取得防止）
- **`grantCompanionGearReward(gearId)`**: フラグ確認→初回のみ付与
- **`reconcileCompanionGearRewards()`**: ロード時に過去クリア済みステージの報酬を自動補完
- **ステータス画面・装備袋UI**: 未所持の特化装備に入手先ステージを表示
- **冒険の記録**: 「🎁 特化装備収集」セクション追加（4/4進捗）
- **デバッグボタン4本 (§109)**: リセット / 全取得 / v2→v3移行確認 / 二重取得防止確認

### Changed
- **`ensureCompanionGearState()`**: v1→v2自動配布ブロックを削除、v<3→v3移行（既所持分をrewardFlags=trueで引き継ぎ）
- **`companionGearVersion`** 目標値: 2 → 3
- ステージ2〜5 ゴールモーダルのrewardLine: 特化装備の取得表示を追加

### セーブデータ互換
- v2セーブ（旧v0.41）: 所持中の特化装備はrewardFlags=trueとして引き継ぎ（装備・所持数変化なし）
- v1セーブ: ステージクリア済みなら`reconcile`で自動補完
...
### Added
- **`COMPANION_LEVEL_MILESTONE_LINES`** (§103): 仲間4人×Lv10/50/99のセリフデータ定数。
- **`checkCompanionLevelMilestones(cid, oldLevel, newLevel)`** (§103): 節目到達チェック・戦闘ログ表示・フラグ更新をまとめた共通関数。複数節目を一度に越えた場合は最高節目のセリフのみ表示し、全通過済み節目をtrueに記録。
- **`getCompanionLevel()` 更新** (§103): `milestones` フィールドが未定義の場合、現在Lvに基づいて補完する（旧セーブ互換）。Lv10→{level10:true} / Lv50→{level10,level50:true} / Lv99→全true。
- **`gainCompanionExp()` 更新** (§103): `oldLevel` をLvアップ前に保存し、アップ後に `checkCompanionLevelMilestones` を呼び出す。
- **ステータス画面「成長の節目」行** (§103): 全4仲間に `Lv10 ✓　Lv50 ・　Lv99 ・` 形式で節目到達状況を表示。成長効果行の直後に配置。
- **デバッグボタン4本** (§103): `🌱 Lv10節目セリフ確認（ジュリタニ）` / `🔥 Lv50節目セリフ確認（シュリタニ）` / `👑 Lv99節目セリフ確認（ノリオ）` / `🔄 仲間節目フラグ全リセット（Lv1・EXP0）`。

### セリフ一覧
- ジュリタニ Lv10: 「ようやく体が旅になじんできた。次はもっと派手にいくぞ！」
- ジュリタニ Lv50: 「ここまで来れば、会心は偶然じゃない。狙って出すものだ！」
- ジュリタニ Lv99: 「最高レベルか。でも、あんたとならまだ先へ行けそうだ！」
- シュリタニ Lv10: 「相手の動きが少し読めるようになってきたよ。」
- シュリタニ Lv50: 「追い詰めるだけじゃだめ。逃げ道まで見てこそ捕獲なの。」
- シュリタニ Lv99: 「捕まえることより、向き合うことの方が大事だって分かったよ。」
- ノリオ Lv10: 「経験って、数字だけじゃないんだな。少し分かってきた。」
- ノリオ Lv50: 「この旅の経験、もうメモ一冊じゃ足りないな。」
- ノリオ Lv99: 「経験値はもう増えなくても、経験は終わらないんだな。」
- ハルミ Lv10: 「小さな光でも、重ねれば誰かを守れるのね。」
- ハルミ Lv50: 「回復するだけじゃない。みんなが進めるように支えるわ。」
- ハルミ Lv99: 「ここまでの旅、全部がひとつの物語になったわ。」

### 変更なし
- `getCompanionGrowthTier()` / `getCompanionGrowthBonus()` / 仲間能力成長値 変更なし。
- 捕獲率 / ノリオEXP2倍 / まかせるAI比率 変更なし。
- `runSingleCompanionAction` / `runCompanionSpecialAction` / `runCompanionMagicAction` 変更なし。
...
### Changed
- **`getCompanionLevel(cid)` 強化** (§100): データ安全化ガードを追加。`level < 1` → 1, `level > 99` → 99, `exp < 0` → 0, `nextExp ≤ 0` → 再計算。旧セーブ・破損データでエラーにならない。
- **`gainCompanionExp()` ログ改善** (§100): Lvアップログを「最終到達Lvのみ1回」に変更。複数Lvアップでもログが長くなりすぎない。Lv99到達時は専用ログ `"🌟 [名前]は最高レベル Lv99 に到達した！"` を出力。
- **Lv99時の表示統一** (§100): 酒場「Lv.99 MAX」/ ステータス EXP行「MAX」/ 冒険の記録「Lv.99 MAX(金色)」に統一。

### Added
- **デバッグボタン3本追加** (§100): 「👥 仲間Lv1・EXP0に戻す」「👥 複数Lvアップ確認（Lv1+EXP500付与）」「👥 パーティ仲間だけEXP確認（2人+2人待機）」。

### 確認済み
- 通常勝利(`winBattle`) / 捕獲成功(`attemptCapture`) でのEXP付与は1回のみ（二重なし）。
- パーティ入れ替え後は新メンバーのみEXP取得（待機中は変化なし）。
- ノリオEXP2倍は主人公のみ。仲間へは `e.exp`（ベース）を付与。
- `doSingUltimateGorilla()` は `gainExp(e.exp)` のみ（仲間EXP対象外・意図通り）。
- `gainExp()` / BGM / 捕獲率 / AI / `partyTrail` 変更なし。


## [0.37] - 2026-07-12 — 仲間成長システム（Lv・EXP 第一段階）(§99)

### Added
- **`state.companionLevels`** (§99): 仲間ごとの `{ level, exp, nextExp }` オブジェクト。state初期化・saveGame・loadGame 対応。
- **`getCompanionLevel(cid)`** (§99): 仲間のLv/EXP状態を取得するヘルパー。初回アクセス時に `{ level:1, exp:0, nextExp:25 }` で初期化。
- **`gainCompanionExp(baseExp)`** (§99): パーティ中の仲間にEXPを付与し、Lv99上限でレベルアップ処理。ログ: `"🎉 [名前]は Lv[N]になった！"`。
- **酒場「仲間を探す」Lv表示** (§99): 各仲間カードヘッダー下に `Lv.N` を水色で表示。
- **酒場「仲間を見る」Lv表示** (§99): パーティ中仲間カードに `Lv.N` を水色で表示。
- **ステータス画面「仲間」セクション強化** (§99): 全4仲間（パーティ外も含む）の `Lv.N / EXP N/M / パーティ状態` を表示。
- **冒険の記録「👥 仲間」セクション追加** (§99): 全4仲間の Lv.N を「次の目標」の手前に表示。パーティ中には ✓ マーク。
- **デバッグボタン3本追加** (§99): 「👥 仲間Lv10にする」「👥 仲間Lv50にする」「👥 仲間Lv99にする」（全4仲間一括）。

### Changed
- **`winBattle()`** (§99): `gainExp(e.exp)` 直後に `gainCompanionExp(e.exp)` を追加。通常勝利時にパーティ仲間もEXP取得。
- **`attemptCapture()`** (§99): 捕獲成功時の `gainExp(e.exp)` 直後に `gainCompanionExp(e.exp)` を追加。

### 確認済み（変更なし）
- `gainExp()` / ノリオEXP2倍効果（主人公のみ）/ BGM 変更なし。
- 捕獲率 / AI / 仲間能力（会心/捕獲/EXP倍率/まほう）変更なし。
- Lv による仲間の能力値強化は未実装（能力は固有ボーナス固定のまま）。
- 究極ゴリラ戦で仲間コマンドなし — 変更なし。
- `partyTrail` / 横スクロール / 図鑑 / 装備 変更なし。
...
### Changed
- **`runCompanionAutoCommand(cid)` 4択対応** (§97): 3択（たたかう / 固有1 / 固有2）から4択（たたかう / 固有1 / 固有2 / **まほう**）に拡張。
  - 仲間別基本比率: ジュリタニ(30/35/15/20%) / シュリタニ(20/35/25/20%) / ノリオ(35/30/15/20%) / ハルミ(20/35/20/25%)。
  - 敵HP≤15 時: ジュリタニ(45/35/5/15%) / シュリタニ(50/25/10/15%) / ノリオ(55/25/5/15%) / ハルミ(70/5/10/15%) に変更。攻撃系まほうも少し残す。
  - ハルミ+HP≤40% 時: たたかう5% / 小さな癒し45% / まもりの光15% / 小さな回復35%。固有回復とまほう回復の両方を使う。
  - ハルミ+HP≥85% 時: たたかう45% / 小さな癒し5% / まもりの光35% / 小さな回復15%。HP満タン付近では回復しすぎない。
  - 前回行動記憶に `"magic"` キーを追加。前回 magic を選んだ場合 wM -0.10 補正 → 正規化で自動配分。
  - ウェイト正規化を `total = wA + wS1 + wS2 + wM` に拡張。total≤0 時は attack フォールバック。
  - magic ログ: `"🤝 名前にまかせた！"` + `"まほう名を選んだ！"` の2行形式（既存と統一）。
  - magic 実行: `runCompanionMagicAction(cid)` → 返値 `true` なら `winBattle()`。ハルミは常に `false` で敵ターンへ。
  - `"special"` 後方互換（= "special1"）を維持。

### Added
- **デバッグボタン3本追加** (§97): 「🎲 まかせるAI 4択確認（仲間4人+のらいぬ）」「🎲 まかせるAI ハルミ回復確認（HP25%）」「🎲 まかせるAI 敵HP10確認（まほう混入）」。

### 確認済み（変更なし）
- `runCompanionMagicAction(cid)` / `runSingleCompanionAction` / `runCompanionSpecialAction` は変更なし。
- `clearCompanionCommandState()` の `lastCompanionAutoAction = {}` リセットは既存のまま動作（magic も含む）。
- 仲間MP / 主人公MP消費なし。
- 捕獲率 / `gainExp()` / BGM制御 変更なし。
- 究極ゴリラ戦（`e.final`）では仲間コマンドが出ないため変更不要。
- UI（仲間コマンド4択 / companion-magic-menu）は変更なし。
...
### Changed
- **`runCompanionAutoCommand(cid)` 比率調整** (§86): 全仲間共通 50/50 → 仲間別比率に変更。ジュリタニ55%/シュリタニ65%/ノリオ50%/ハルミ70% で固有コマンドを選ぶ。
- **`runCompanionSpecialAction(cid)` 数値調整** (§86):
  - ジュリタニ「会心の構え」: ダメージ上限 30→28、会心率 35%→30%（倍率×1.6 維持）。ログを2行形式に統一。
  - シュリタニ「捕獲アシスト」: ダメージ 固定1 → Lv連動 `Math.min(3, 1+floor(Lv/30))`（1〜3）。ログ「捕まえやすい間合いを作った！」に整理。
  - ノリオ「経験値の眼」: ダメージ上限 10→12。ログ「この戦い、学びが多そうだ！」に整理。
  - ハルミ「小さな癒し」: 回復上限 30→25。HP満タン時は「しかし、HPはすでに満タンだ。」を表示。
...
### Added
- **`state.partyTrail`** (§78): 状態オブジェクトに軌跡配列を追加。最大2エントリ `{x, y}`。ロード時・マップ切り替え時にリセット。
- **デバッグ §78**: 「🚶 仲間2人をパーティに追加」+ 「🔄 仲間軌跡リセット」の2ボタン。

### Changed
- **`movePlayer(dx, dy)`** (§78): 境界/ブロックチェック通過後、`p.x/y` 更新前に現在位置を `state.partyTrail.unshift()` で追加。2件超で `pop()`。
- **`renderField()`** (§78): タイルループ前に `trailMap` を構築（仲間0優先の逆順ループ）。タイル描画の判定順を「プレイヤー → 仲間追従 → アイテム → 地形」に拡張。
- **`switchToSideMap()`** (§78): サイドマップ入場時に `state.partyTrail = []`。
- **`switchToNormalMap()`** (§78): 通常マップ帰還時に `state.partyTrail = []`。
- **`loadGame()`** (§78): セーブデータロード時に `state.partyTrail = []`（軌跡は永続化しない）。

### Notes
- 仲間0がtrail[0]に、仲間1がtrail[1]に対応。2人が同座標になった場合は仲間0優先。プレイヤーと同座標の場合は非表示。
- 横スクロールマップ（`mapMode === "side"`）では `renderField()` が `renderSideField()` に委譲するため追従描画は発生しない。
- 仲間能力・加入/離脱処理・パーティ上限・戦闘・BGM は一切変更なし。

## [0.25.1] - 2026-07-11 — デバッグUIラベル整理・将来候補ドキュメント追記

### Changed
- **デバッグボタンラベル**: 「👥 仲間セリフ: 〜」→「👥 仲間UI：〜」に全統一。「クリアのみ」→「クリア済み」に表記修正。

### Docs
- **TODO.md**: `## 🔮 今後の実装候補` セクション追加。v0.26（フィールド仲間追従）/ v0.27（仲間戦闘自動参加）/ v0.28（仲間コマンド選択）を記録。
- **GAME_DESIGN.md**: `## 将来の実装候補（v0.26〜v0.28）` セクション追加。各候補の概要・影響範囲・前提条件を記載。
- **HANDOFF.md**: 「次の推奨実装順」をv0.26〜v0.28候補に更新。プレイヤーフィードバック出典を明記。
